...
function mapTweet(tweet: TweetV2, includes?: ApiV2Includes): Tweet {
  ...
  const medias =
    includes?.media?.filter((m) => mediaKeys.includes(m.media_key)) || [];
  ...
  medias.forEach((m) => {
    if (m.type === 'photo' && m.url) {
      ...
      // m.url is sometimes variant url
  ...
  return {
    ...
    html: undefined,
    permanentUrl: `https://twitter.com/${author?.username || 'i'}/status/${
      tweet.id
    }`,
...
app.get('/tweets/:username', async (req: Request, res: Response) => {
  try {
    ...
    const tweets =
      timeline.data.data?.map((t) => mapTweet(t, timeline.data.includes)) || [];
...
app.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    const tweets =
      search.data.data?.map((t) => mapTweet(t, search.data.includes)) || [];
